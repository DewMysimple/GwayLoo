...
for (const viewport of viewports) {
  ...
  // Source TZ scheduling keeps the first paper behind its 1.5s intro delay;
  // leave a small frame margin before pointer/raycast probes begin.
  await sleep(1800);
  ...
  const paperLayerState = await evaluate(`(()=>{const definition=window.__xp.experienceDefinition;const papers=definition.world.papers;const layers=definition.world.paperLayers;const aligned=layers.ground.every((entry,index)=>entry.index===index&&entry.name===papers[index]?.name&&entry.startAt===papers[index]?.startAt&&entry.sceneIndex===papers[index]?.sceneIndex)&&layers.sdf.every((entry,index)=>entry.index===index&&entry.name===papers[index]?.name)&&layers.vegetation.every((entry,index)=>entry.index===index&&entry.name===papers[index]?.name)&&layers.shadow.every((entry,index)=>entry.index===index&&entry.name===papers[index]?.name)&&layers.presentation.every((entry,index)=>entry.index===index&&entry.name===papers[index]?.name&&entry.sceneIndex===papers[index]?.sceneIndex);return {groundCount:layers.ground.length,groundEnabledCount:layers.ground.filter(entry=>entry.hasGround).length,sdfCount:layers.sdf.length,sdfFadeCount:layers.sdf.filter(entry=>entry.revealType==='fade').length,sdfTransparentCount:layers.sdf.filter(entry=>entry.transparency).length,vegetationCount:layers.vegetation.length,vegetationLeavesCount:layers.vegetation.filter(entry=>entry.leaves).length,shadowCount:layers.shadow.length,shadowCastCount:layers.shadow.filter(entry=>entry.castShadow).length,shadowHoleCount:layers.shadow.filter(entry=>entry.hasHole).length,presentationCount:layers.presentation.length,presentationTitleCount:layers.presentation.filter(entry=>entry.title).length,presentationCtaCount:layers.presentation.filter(entry=>entry.cta).length,aligned,sdfContract:layers.sdf.every(entry=>typeof entry.sdf?.min==='number'&&['default','fade'].includes(entry.revealType)&&typeof entry.transparency==='boolean'),vegetationContract:layers.vegetation.every(entry=>typeof entry.hasHoverEffect==='boolean'),shadowContract:layers.shadow.every(entry=>typeof entry.castShadow==='boolean'&&typeof entry.hasHole==='boolean'),groundContract:layers.ground.every(entry=>typeof entry.hasGround==='boolean'&&typeof entry.ground?.depth==='number'),presentationContract:layers.presentation.every(entry=>typeof entry.sceneIndex==='number'&&(entry.title===undefined||typeof entry.title==='string')&&(entry.cta===undefined||typeof entry.cta==='string'))};})()`);
  const deviceState = await evaluate(`(()=>{const definition=window.__xp.experienceDefinition;const manager=window.__xp.experienceManager;const view=manager._watercolorView;const device=definition.assets.device;const videoResources=definition.assets.videoResources;return {device,videoResourceCount:videoResources.length,videoResourcesMatch:videoResources.every(item=>item.path.includes('/videos/'+device+'/')),grassVisible:view.grassLayer.group.visible,grassPatchCount:view.grassLayer.configs.length,leavesEnabled:view.leavesLayer?.enabled??false,leavesPositionPass:!!view.leavesLayer?._positionMaterial,longPressTime:manager._paintManager?._longPressTime??null,scrollTouchParallaxDisabled:view.scrollCamera.getDebugState?.().touchParallaxDisabled??null};})()`);
  const caseDir = resolve(outputRoot, viewport.name);
  const layerState = await evaluate(`(()=>{const manager=window.__xp.experienceManager;const view=manager._watercolorView;const papers=view.papers;const revealSchedule=view.getRevealSchedule?.()??[];const revealScheduleMatches=revealSchedule.length===papers.length&&revealSchedule.every((entry,index)=>{const priorSameStart=revealSchedule.slice(0,index).filter(other=>Math.abs(other.startAt-entry.startAt)<1e-6).length;const expectedDelay=(entry.paperIndex===0?1.5:0)+priorSameStart*.3;return Math.abs(entry.delay-expectedDelay)<1e-6&&Math.abs(entry.triggerAt-entry.startAt)<1e-6&&Math.abs(entry.animationStartAt-entry.startAt-entry.delay)<1e-6;});const regions=[...manager._simulation._regions.values()];const fullPaintRegion=regions.find(region=>region.paperIndex===manager._simulation.fullPaintPaperIndex);let overlaps=0;for(let i=0;i<regions.length;i++)for(let j=i+1;j<regions.length;j++){const a=regions[i],b=regions[j];if(a.x<b.x+b.width&&a.x+a.width>b.x&&a.y<b.y+b.height&&a.y+a.height>b.y)overlaps++;}const paperYawAlignment=Math.max(...papers.map(p=>{const q=p.mesh.getWorldQuaternion(p.mesh.quaternion.clone());const worldYaw=Math.atan2(2*(q.w*q.y+q.x*q.z),1-2*(q.y*q.y+q.z*q.z));return Math.abs(p.transform.rotation.y+worldYaw);}));const paperWorldPositionAlignment=Math.max(...papers.map(p=>{const w=p.mesh.getWorldPosition(p.mesh.position.clone());return Math.hypot(p.transform.position.x-w.x,p.transform.position.y-w.y,p.transform.position.z-w.z);}));const sourceWorldScaleFinite=papers.every(p=>[p.sourceWorldScale.x,p.sourceWorldScale.y,p.sourceWorldScale.z].every(Number.isFinite));const cutout=view.cutoutShadow;const cutoutMaterial=cutout?._material;const cutoutGeometry=cutout?._mesh?.geometry;const ground=view._groundMesh;const groundMaterial=view._groundMaterial;const groundGeometry=ground?.geometry;const groundUniforms=groundMaterial?.uniforms;return {paperCount:papers.length,premature:papers.filter(p=>p.identity.startAt>0&&(p.revealed||Math.abs(p.state.rotationZ+Math.PI/2)>.001)).length,revealScheduleCount:revealSchedule.length,revealScheduleMatches,completeLayerBaseline:view._paperMaterial?.uniforms.uCompleteLayerBaseline?.value??0,revealTiming:view.getRevealTiming?.()??null,titleCount:view.paintingTitles.configs.length,simulationRegions:regions.length,simulationAtlasSize:manager._simulation.atlasSize,simulationUniqueSizes:new Set(regions.map(r=>r.width+'x'+r.height)).size,simulationOverlaps:overlaps,fullPaintRegion:fullPaintRegion?{width:fullPaintRegion.width,height:fullPaintRegion.height,ratio:fullPaintRegion.ratio}:null,grassPatches:view.grassLayer.configs.length,grassBounds:view.grassLayer._patches[0]?.mesh.geometry.boundingSphere?.radius??0,paperYawAlignment,paperWorldPositionAlignment,sourceWorldScaleFinite,shadowSources:view.shadowProjection._sources.length,shadowTexture:view.shadowProjection.texture.name,groundBatch:!!ground&&ground.count===papers.length&&ground.name==='SourceGroundsBatch',groundBatchAttributes:!!groundGeometry?.getAttribute?.('instance')&&!!groundGeometry?.getAttribute?.('size')&&!!groundGeometry?.getAttribute?.('simulationBox')&&!!groundGeometry?.getAttribute?.('simulationRemap'),groundBatchUniforms:!!groundUniforms?.uVisible&&!!groundUniforms?.uAlpha&&Array.isArray(groundUniforms?.uAtlasRemap?.value),cutoutShadowSources:cutout?._sources.length??0,cutoutShadowMesh:!!cutout?._mesh,cutoutShadowAttributes:!!cutoutGeometry?.getAttribute?.('aSdfAtlasRemap')&&!!cutoutGeometry?.getAttribute?.('aSdfScale')&&!!cutoutGeometry?.getAttribute?.('aAlpha'),cutoutShadowShader:!!cutoutMaterial?.fragmentShader?.includes('uShadowMap')&&!!cutoutMaterial?.fragmentShader?.includes('uCutoutShadowIntensity'),cutoutShadowUniforms:!!cutoutMaterial?.uniforms?.uShadowMap&&!!cutoutMaterial?.uniforms?.uSdfTexture};})()`);
  ...
  const cameraState = await evaluate(`(async()=>{const camera=window.__xp.experienceManager._watercolorView.scrollCamera;const wait=ms=>new Promise(resolve=>setTimeout(resolve,ms));const move=(x,y)=>window.dispatchEvent(new PointerEvent('pointermove',{clientX:x,clientY:y,pointerType:'mouse',bubbles:true}));await wait(250);move(innerWidth*.5,innerHeight*.5);await wait(250);const center=camera.camera.position.toArray();move(innerWidth-2,innerHeight-2);await wait(500);const edge=camera.camera.position.toArray();move(innerWidth*.5,innerHeight*.5);const positionDelta=Math.max(...center.map((value,index)=>Math.abs(value-edge[index])));return {...camera.getDebugState?.(),pointerPositionDelta:positionDelta,pointerPositionInvariant:positionDelta<.0001};})()`);
  ...
  const layerDurations = await evaluate(`(()=>{const view=window.__xp.experienceManager._watercolorView;const paper=view.papers.find(entry=>!entry.revealed&&entry.identity.startAt>0);if(!paper)return null;view._reveal(paper);const tweens=paper.tween.getChildren(true,true,false);const result={rise:tweens.find(tween=>tween.vars.rotationZ===0)?.duration()??null,curve:tweens.find(tween=>tween.vars.curve===1)?.duration()??null,reveal:tweens.find(tween=>tween.vars.reveal===15)?.duration()??null};paper.tween.kill();view.hideAll({immediate:true});const fade=view.papers.find(entry=>entry.sdf.revealType==='fade');view._reveal(fade);const fadeTweens=fade.tween.getChildren(true,true,false);const fadeResult={rotationZ:fade.state.rotationZ,curve:fade.state.curve,alphaDuration:fadeTweens.find(tween=>tween.vars.alpha===1)?.duration()??null,hasRise:fadeTweens.some(tween=>tween.vars.rotationZ===0&&tween.duration()>0.1)};fade.tween.kill();view.hideAll({immediate:true});return {...result,fade:fadeResult};})()`);
  const groundLifecycle = await evaluate(`(async()=>{const view=window.__xp.experienceManager._watercolorView;const ground=view._grounds[0];const paper=ground&&view.papers[ground.paperIndex];const batch=view._groundMesh;const material=view._groundMaterial;if(!ground||!paper||!batch||!material)return null;const initial=material.uniforms.uAlpha.value[ground.paperIndex];view._reveal(paper);const visibleBeforeDelay=material.uniforms.uVisible.value[ground.paperIndex];await new Promise(resolveDelay=>setTimeout(resolveDelay,paper.revealDelay*1000+80));const visibleAfterDelay=material.uniforms.uVisible.value[ground.paperIndex];const tweens=paper.tween.getChildren(true,true,false);const result={initial,batchCount:batch.count,visible:visibleAfterDelay,visibleBeforeDelay,visibleAfterDelay,alphaDuration:tweens.find(tween=>tween.vars.uAlpha===1)?.duration()??null};paper.tween.kill();view.hideAll({immediate:true});return result;})()`);
  const hideLifecycle = await evaluate(`(()=>{const view=window.__xp.experienceManager._watercolorView;const paper=view.papers.find(entry=>entry.identity.startAt>0)??view.papers[0];if(!paper)return null;view.hideAll({immediate:true});paper.revealed=true;paper.state.alpha=1;const timeline=view.hideAll();const start={alpha:paper.state.alpha,revealed:paper.revealed,duration:timeline.duration()};timeline.time(.25);const mid={alpha:paper.state.alpha,revealed:paper.revealed};timeline.progress(1);const done={alpha:paper.state.alpha,revealed:paper.revealed,groundVisible:view._groundVisible[paper.index]??false};view.hideAll({immediate:true});return {start,mid,done};})()`);
  ...
  const sourceMaterialState = await evaluate(`(()=>{const u=window.__xp.experienceManager._watercolorView._paperMaterial?.uniforms;const l=u?.uLighting?.value;const b=u?.uBackground?.value;const base=u?.uNoiseTexture?.value;const final=u?.uNoiseFinalTexture?.value;return {lighting:l?{groundScale:[l.groundSpecularScale.x,l.groundSpecularScale.y],groundOffset:[l.groundSpecularOffset.x,l.groundSpecularOffset.y],groundStrength:l.groundSpecularStrength,specularCenter:[l.specularCenter.x,l.specularCenter.y],specularScale:[l.specularScale.x,l.specularScale.y],specularStrength:l.specularStrength}:null,background:b?{ground:b.groundColor.getHexString(),sky:b.skyColor.getHexString(),progress:[b.progressRemap.x,b.progressRemap.y]}:null,noiseWrap:{base:[base?.wrapS??null,base?.wrapT??null],final:[final?.wrapS??null,final?.wrapT??null]}};})()`);
  const backgroundCompositeState = await evaluate(`(()=>{const p=window.__xp.experienceManager._watercolorView.shadowProjection;const u=p?._compositeMaterial?.uniforms;const value=name=>u?.[name]?.value;const shader=p?._compositeMaterial?.fragmentShader??'';return {noise:!!value('tNoiseTexture'),projectionInverse:value('uProjectionInverse')?.elements?.length??0,viewInverse:value('uViewMatrixInv')?.elements?.length??0,view:value('uViewMatrix')?.elements?.length??0,ratio:value('uRatio')??0,resolution:value('uResolution')?[value('uResolution').x,value('uResolution').y]:null,fog:value('uFogState')?[value('uFogState').x,value('uFogState').y]:null,lightingUniform:!!u?.uLighting,backgroundUniform:!!u?.uBackground,sourceShader:shader.includes('computeLighting(uLighting, uRatio, vUv)')&&shader.includes('computeBackground(uBackground, vUv)')&&!shader.includes('uShadowMap'),shadowUniform:!!u?.uShadowMap};})()`);
  ...
  const fluidPassState = await evaluate(`(()=>{const manager=window.__xp.experienceManager;const simulation=manager._simulation;const divergence=simulation?._divergenceMat?.fragmentShader??'';const pressure=simulation?._pressureMat?.fragmentShader??'';const gradient=simulation?._gradientMat?.fragmentShader??'';const vertex=simulation?._divergenceMat?.vertexShader??'';const advection=simulation?._advect?.fragmentShader??'';const splat=simulation?._splat?.fragmentShader??'';const paperShader=manager._watercolorView?._paperMaterial?.fragmentShader??'';const fullPaintShader=manager._fullPaintManager?._material?.fragmentShader??'';const states=[...(simulation?._states?.values?.()??[])];const geometry=simulation?._mesh?.geometry;const cellScale=geometry?.getAttribute?.('aCellScale');const cellScaleValues=cellScale?Array.from(cellScale.array):[];const expectedCellScale=states.every(state=>Math.abs(cellScaleValues[state.paperIndex*2]-(1/state.fboSize.x))<.000001&&Math.abs(cellScaleValues[state.paperIndex*2+1]-(1/state.fboSize.y))<.000001);const dtScaling=states.length>0&&states.every(state=>Math.abs(state.dt-.008*state.fboSize.x/simulation.atlasSize)<.000001);return {divergenceUsesDt:divergence.includes('* 0.5 / max(vDt, 0.000001)'),divergenceUsesSingleCell:divergence.includes('vCellScale.x')&&divergence.includes('vCellScale.y')&&!divergence.includes('uTexelSize'),pressureUsesPerInstanceCellScale:pressure.includes('vCellScale.x')&&pressure.includes('vCellScale.y')&&!pressure.includes('uTexelSize'),gradientUsesPerInstanceCellScale:gradient.includes('vCellScale.x')&&gradient.includes('vCellScale.y')&&!gradient.includes('uTexelSize'),vertexCarriesCellScale:vertex.includes('aCellScale')&&vertex.includes('vCellScale'),cellScaleAttribute:!!cellScale,expectedCellScale,advectionUsesV2:advection.includes('spotOld2')&&advection.includes('spotNew3')&&advection.includes('noise2.r * 3.0')&&advection.includes('vDt * ratio'),splatUsesSourceEllipse:splat.includes('sdEllipse')&&splat.includes('uCurrentScale')&&splat.includes('uPaperRatio')&&splat.includes('vWasActive < 0.5')&&!splat.includes('sdUnevenCapsule'),paperUsesDeliveryGain:paperShader.includes('const float DELIVERY_SIMULATION_VISUAL_GAIN = 0.015;')&&paperShader.includes('visibleVel = vel * DELIVERY_SIMULATION_VISUAL_GAIN'),paperSimulationUvKeepsX:manager._watercolorView?._paperMaterial?.vertexShader?.includes('vec2(uv.x, 1. - uv.y)')??false,dtScaling,fullPaintSourceAlpha:fullPaintShader.includes('gl_FragColor = inkColor;')&&!fullPaintShader.includes('inkColor.a * uAlpha')};})()`);
  ...
  const brushSample = await evaluate(`(()=>{const manager=window.__xp.experienceManager;const s=manager._paintManager.lastBrushSample;if(!s)return null;const accumulation=manager._simulation.readAccumulation(s.paperIndex,s.currentUv);return {paperIndex:s.paperIndex,visibleDiameter:s.visibleDiameter,sourceScreenSpaceSize:s.sourceScreenSpaceSize,sourceRadiusX:s.currentRadius.x,sourceRadiusY:s.currentRadius.y,sourceEllipseRatio:s.currentRadius.y/Math.max(s.currentRadius.x,.0001),projectedSize:s.projectedSize,simulationSize:s.simulationSize,pressed:s.pressed,accumulation};})()`);
  ...
  if (!viewport.mobile && viewport.width === 1440) {
    ...
    await evaluate(`(()=>{const view=window.__xp.experienceManager._watercolorView;view.papers.forEach(paper=>paper.tween?.kill());view.hideAll({immediate:true});})()`);
  }
  ...
  const fullPaintVideoState = await evaluate(`(()=>{const definition=window.__xp.experienceDefinition;const manager=window.__xp.experienceManager._fullPaintManager;const sources=[...manager._videoCache.values()].map(video=>video.src);return {device:definition.assets.device,sources,sourcesMatch:sources.length>0&&sources.every(src=>src.includes('/videos/'+definition.assets.device+'/'))};})()`);
  ...
  cases.push({ ...viewport, runtimeState, definitionContent, paperLayerState, deviceState, layerState, cursorState, cameraState, leavesState, leavesLifecycle, fullPaintLifecycle, uvMapping, layerDurations, groundLifecycle, hideLifecycle, cutoutLifecycle, renderResolution, sourceMaterialState, backgroundCompositeState, globalGroundState, fluidPassState, fluidStencilState, fluidActivityState, idleHoverState, scrollStep, scrollResponse, rippleScene, smallBrushSample, largeBrushSample, brushSample, brushSlowMotion, brushMotion, grassLifecycle, slowMotion, timings, titleClick, fullPaintVisible, fullPaintState, fullPaintVideoState, fullPaintRuntimeState, poemOpening, poemHiding, poemClosed, fullPaintSoftFailure, fullPaintFailure, finalSlot, restartY });
}
...
const report = {
  ...
  passed: reducedFallback.isFallback
    ...
    && cases.every((testCase) => testCase.restartY === 0
      ...
      && testCase.layerState.revealScheduleCount === 26
      && testCase.layerState.revealScheduleMatches
      ...
      && Math.abs(testCase.layerState.fullPaintRegion?.ratio - (testCase.width / testCase.height)) < 0.02
      ...
      && testCase.layerState.paperWorldPositionAlignment < 0.0001
      && testCase.layerState.sourceWorldScaleFinite
      ...
      && testCase.cameraState?.pointerNdcLimit === 0.2
      && testCase.cameraState?.touchParallaxDisabled === testCase.mobile
      && testCase.cameraState?.pointerPositionInvariant
      ...
      && !testCase.groundLifecycle?.visibleBeforeDelay
      && testCase.groundLifecycle?.visibleAfterDelay
      && Math.abs(testCase.groundLifecycle?.alphaDuration - 0.4) <= 0.1
      && Math.abs(testCase.hideLifecycle?.start?.duration - 0.5) <= 0.001
      && testCase.hideLifecycle?.start?.alpha > 0.99
      && testCase.hideLifecycle?.mid?.alpha > 0
      && testCase.hideLifecycle?.mid?.alpha < 1
      && testCase.hideLifecycle?.mid?.revealed
      && testCase.hideLifecycle?.done?.alpha === 0
      && !testCase.hideLifecycle?.done?.revealed
      && !testCase.hideLifecycle?.done?.groundVisible
      ...
      && testCase.sourceMaterialState?.background?.sky === "e4e4e4"
       ...
       && testCase.sourceMaterialState?.noiseWrap?.base?.[0] === 1000
       && testCase.sourceMaterialState?.noiseWrap?.base?.[1] === 1000
       && testCase.sourceMaterialState?.noiseWrap?.final?.[0] === 1002
       && testCase.sourceMaterialState?.noiseWrap?.final?.[1] === 1002
      ...
      && testCase.backgroundCompositeState?.ratio > 0
      && testCase.backgroundCompositeState?.lightingUniform
      && testCase.backgroundCompositeState?.backgroundUniform
      && testCase.backgroundCompositeState?.sourceShader
      ...
      && testCase.fluidPassState?.pressureUsesPerInstanceCellScale
      && testCase.fluidPassState?.gradientUsesPerInstanceCellScale
      && testCase.fluidPassState?.vertexCarriesCellScale
      && testCase.fluidPassState?.cellScaleAttribute
      && testCase.fluidPassState?.expectedCellScale
      && testCase.fluidPassState?.advectionUsesV2
      && testCase.fluidPassState?.splatUsesSourceEllipse
      && testCase.fluidPassState?.paperUsesDeliveryGain
      && testCase.fluidPassState?.paperSimulationUvKeepsX
      ...
      && testCase.paperLayerState?.presentationCount === 26
      && testCase.paperLayerState?.presentationTitleCount === 6
      && testCase.paperLayerState?.presentationCtaCount === 6
      && testCase.deviceState?.device === (testCase.mobile ? "mobile" : "desktop")
      && testCase.deviceState?.videoResourceCount === 12
      && testCase.deviceState?.videoResourcesMatch
      && testCase.deviceState?.grassVisible === !testCase.mobile
      && testCase.deviceState?.leavesEnabled === !testCase.mobile
      && testCase.deviceState?.leavesPositionPass === !testCase.mobile
      && Math.abs(testCase.deviceState?.longPressTime - (testCase.mobile ? 0.6 : 0.3)) < 0.0001
      && testCase.deviceState?.scrollTouchParallaxDisabled === testCase.mobile
      && testCase.fullPaintVideoState?.device === testCase.deviceState?.device
      && testCase.fullPaintVideoState?.sourcesMatch
      ...
      && testCase.paperLayerState?.presentationContract
      ...
      && (testCase.mobile || (testCase.largeBrushSample?.paperIndex === 0
        ...
        && testCase.smallBrushSample?.sourceScreenSpaceSize > 0
        && testCase.largeBrushSample?.sourceScreenSpaceSize > 0
        && testCase.largeBrushSample?.sourceRadius > testCase.smallBrushSample?.sourceRadius
        && testCase.brushSample?.sourceEllipseRatio <= 1.15
