...
function getTextRect(isMobile: boolean): { x: number; y: number; w: number; h: number } {
  const measured = document.querySelector<HTMLElement>(".xp-text-sizer")?.getBoundingClientRect();
  const w = measured?.width || Math.min(window.innerWidth * 0.8, 13 * (isMobile ? 25 : 30));
  const h = measured?.height || (isMobile ? 360 : 432);
  const x = measured?.left ?? window.innerWidth * 0.1;
