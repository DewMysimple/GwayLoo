(function($) {
  $(document).ready(function() {
    ...
    // All checkout-state bookkeeping is stored per form (as form data), so
    // multiple signup forms on the same page update and settle independently.
    // Key flags: `meprCheckoutStateUpdating` blocks submission until the state is
    // reconciled, `meprUpdateCheckoutStatePending` marks that a newer update is
    // scheduled but has not run yet, and `meprUpdatingCheckoutState` guards a
    // single in-flight request.
    const updateCheckoutState = function ($form) {
      clearTimeout($form.data('meprUpdateCheckoutStateTimeout'));

      // Block form submission until the checkout state is reconciled, and record
      // that a newer update is scheduled so an earlier request's completion does
      // not settle (and release a queued submit) before this update has run.
      $form.data('meprCheckoutStateUpdating', true);
      $form.data('meprUpdateCheckoutStatePending', true);

      $form.data('meprUpdateCheckoutStateTimeout', setTimeout(function () {
        debouncedUpdateCheckoutState($form);
      }, 50));
    };

    const settleCheckoutStateUpdate = function ($form) {
      // The completed request has finished its retry lifecycle, so clear the
      // retry flag even when a newer update is still pending: the next scheduled
      // update gets its own retry budget.
      $form.removeData('meprUpdateCheckoutStateRetried');

      // A newer update is still scheduled; keep the submission blocked until it runs
      // so the form settles against the latest checkout state, not an earlier one.
      if ($form.data('meprUpdateCheckoutStatePending')) {
        return;
      }

      $form.removeData('meprCheckoutStateUpdating');

      if ($form.data('meprSubmitWhenUpdated')) {
        $form.removeData('meprSubmitWhenUpdated');

        // A disabled button would swallow the triggered click.
        $form.find('.mepr-submit').prop('disabled', false).trigger('click');
      }
    };

    const getRequestData = function ($form) {
      let payment_methods = [];
    ...
    const debouncedUpdateCheckoutState = function ($form) {
      // The scheduled update is now running, so it is no longer pending.
      $form.removeData('meprUpdateCheckoutStatePending');

      // A request is already in flight. Defer via the "Again" flag; when that
      // request completes it re-runs updateCheckoutState, which re-asserts the
      // pending marker, so the submit lock is never dropped in the meantime.
      if ($form.data('meprUpdatingCheckoutState')) {
        $form.data('meprUpdateCheckoutStateAgain', true);
        return;
      }

      const data = getRequestData($form);

      if (JSON.stringify(data) === JSON.stringify($form.data('meprLastRequestData'))) {
        settleCheckoutStateUpdate($form);
        return;
      }

      $form.data('meprUpdatingCheckoutState', true);
      $form.data('meprLastRequestData', data);
      ...
      const onComplete = function () {
        ...
        $form.removeData('meprUpdatingCheckoutState');
      };
      ...
      .done(function (response) {
        if (response && typeof response.success === 'boolean' && response.success) {
          ...
          if ($form.data('meprUpdateCheckoutStateAgain')) {
            $form.removeData('meprUpdateCheckoutStateAgain');
      ...
      .fail(function () {
        ...
        // Retry a failed update once before releasing any queued submission, so
        // a transient error does not let the submit proceed unreconciled.
        if (!$form.data('meprUpdateCheckoutStateRetried')) {
          $form.data('meprUpdateCheckoutStateRetried', true);
          $form.removeData('meprLastRequestData');
          updateCheckoutState($form);
          return;
        }
    ...
    $('.mepr-signup-form').on('meprUpdateCheckoutState', function (e, force) {
      // A forced update bypasses the identical-request check, for when the
      // checkout state changed server-side without any form fields changing.
      if (force) {
        $(this).removeData('meprLastRequestData');
      }
    ...
    var meprValidateInput = function (obj, submitting) {
      ...
      if($(obj).hasClass('mepr-coupon-code') && !submitting) {
        ...
        if($(obj).val().match(/(\s|\S)/)) {
          ...
          .fail(function () {
            // Also reached when the coupons feature is stripped and the
            // mepr_validate_coupon endpoint no longer exists.
            $(obj).prev('.mp-form-label').find('.mepr-coupon-loader').hide();
            mpToggleFieldValidation($(obj), false);
            $(obj).removeData('validating');
            $(obj).removeData('submit-when-valid');
          });
        }
    ...
    $('body').on('click', '.mepr-signup-form .mepr-submit', function (e) {
      ...
      // Wait for an in-flight checkout state update, so the payment fields are
      // reconciled with the final order before the form is submitted.
      if(form.data('meprCheckoutStateUpdating')) {
        form.data('meprSubmitWhenUpdated', true);
        return;
      }
    ...
    var handleOrderBumpToggle = function () {
      ...
      var $payment_method_inputs = $form.find('div[class^=mepr-payment-method] input.mepr-form-radio, input[type="hidden"][name="mepr_payment_method"]');

      $payment_method_inputs.each(function () {
        var $pm = $(this);
      ...
      if(!$to_show.length) {
        if( is_ob_required ) {
          // A real problem only when payment methods exist but none are compatible with the
          // required bump. When none are rendered the checkout collects no payment (e.g. a
          // coupon zeroed the total), so a required bump needs no gateway and submit proceeds.
          if( $payment_method_inputs.length ) {
