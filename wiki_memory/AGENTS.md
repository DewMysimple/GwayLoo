...
本目录是网页主线记忆。Blender 工作台与网页在同一个 `main` 工程中开发，但 Blender 专属记忆位于 `blender_scenebench/wiki_memory/`；混合任务按范围分别写入两套记忆，不把工作台细节重新堆回本目录。
...
- R3F 逐帧视觉等价前，不删除 `LegacyRuntimeBridge`、旧脚本、旧样式、隐藏契约节点或 `/wp-content/` 兼容路径。
- 外部参考基准不是公开仓库依赖；如需对照测试，维护者通过 `GWAYLOO_REFERENCE_ROOT` 显式提供只读目录，只能启动、读取和测量，不能修改。
...
- 验证顺序为 `npm run lint`、`npm run typecheck`、`npm run test`、`npm run check:assets`、`npm run build`、`npm run test:e2e`；不使用 `file://`。
