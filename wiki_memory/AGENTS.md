...
- 默认运行时是 React Three Fiber；`?runtime=legacy` 才由 `LegacyRuntimeBridge` 加载 `public/wp-content/themes/davidwhyte/app.js` 作为短期回退。
- 新运行时只能读取 `public/assets/`；人工视觉验收前暂不删除 legacy 代码与 `/wp-content/`，验收标准是行为等价与 Verminoble 新视觉，不是逐帧复刻。
- `C:/Users/Administrator/Desktop/网页(1)` 是只读源码与交互事实基准，只能启动、读取和测量，不能修改。
...
- 资源替换优先编辑 `src/content/assets.ts` 与 `src/content/scenes.ts`；视频保持 `desktop/mobile`、`base/over` 和 `1-6` 的可管理映射。
- 验证顺序为 `npm run lint`、`npm run typecheck`、`npm run test`、`npm run check:architecture`、`npm run check:assets`、`npm run build`、`npm run test:e2e`；不使用 `file://`。
